...
const { Tapable, SyncHook, SyncWaterfallHook, AsyncSeriesWaterfallHook } = require("tapable");
...
class BundleEnsureWebpackPlugin extends Tapable {
  ...
  getManifestForEntry(entrypoint, compilation){
    ...
    var externalsUrl = options.externalsUrl;
    var manifest = [];
    ...
    externals.forEach((external,name) => {
      var url = "";
      if(typeof externalsUrl == "object" && externalsUrl[name]){
        url = externalsUrl[name];
      }
      manifest.push({
        isExternal:true,
        name:name,
    ...
    this.prepareChunks.forEach((chunk, id) => {
      ...
      var chunkUrl = publicPath + filename;
      manifest.push({
        isChunk:true,
        id:id,
        name:chunk.name,
