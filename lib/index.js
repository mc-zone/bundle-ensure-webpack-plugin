...
class BundleEnsureWebpackPlugin extends Tapable {
  constructor(options){
    ...
    if(typeof this.options.retryTemplate === "undefined"){
      this.options.retryTemplate = fs.readFileSync(path.resolve(__dirname, "../template/retry/default.js"), "utf8");
    }
  ...
  getManifestForEntry(entrypoint, compilation){
    ...
    var defaultExternalPublicPath;
    if(typeof options.externalPublicPath == "string"){
      defaultExternalPublicPath = options.defaultExternalPublicPath;
    }else{
      defaultExternalPublicPath = publicPath;
    }
    if(defaultExternalPublicPath.length && !/\/$/.test(defaultExternalPublicPath)){
      defaultExternalPublicPath += "/";
    }
    ...
    externals.forEach((external,name) => {
      var url;
      if(typeof options.externalPublicPath == "object" && options.externalPublicPath[name]){
        url = options.externalPublicPath[name];
      }else{
        url = defaultExternalPublicPath + name + ".js";
      }
      manifest.push({
        isExternal:true,
        name:name,
  ...
  renderStartupScript(compilation, manifest){
    var options = this.options;
    var jsonpFunctionName = this.getJsonpFunctionName(compilation);
